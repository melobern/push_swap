...
/*   Updated: 2024/02/01 14:02:24 by mbernard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (sorted_after_first(pile_a))
		first_sort(pile_a, len);
	if (sorted_after_three_first(pile_a))
		three_first_sort(pile_a, len);
	else
		push_into_b(pile_a, pile_b, len);
}
...
int	main(int ac, char **av)
{
	...
	pile_a = NULL;
	pile_b = NULL;
	pile_len = fill_pile(&pile_a, av + 1);
	if (!pile_sorted(&pile_a))
	{
		if (pile_a_len == 2)
			...
			push_swap(&pile_a, &pile_b, pile_len);
	}
	// CRAPPY TESTS		////////////////////
